---
layout: post
title:  "【メモ】Vue.jsアプリ構成の調査で参考になった記事一覧"
description: "【メモ】Vue.jsアプリ構成の調査で参考になった記事一覧"
date: 2018-02-07
categories: engineering
lang: ja_JP
tags:
- vue
---

## Vue
- [Vue](https://jp.vuejs.org/index.html)
- [Vue router](https://router.vuejs.org/ja/)
- [Vuex](https://vuex.vuejs.org/ja/)
- [Vue loader](https://vue-loader.vuejs.org/ja/#)
- [Vue.jsとAxiosなら驚くほど簡単に作れる！外部APIを使ったWebアプリの実例](https://www.webprofessional.jp/fetching-data-third-party-api-vue-axios/)

## ライブラリ
- [Lodash/Underscoreは必要ない（かも）](https://qiita.com/ossan-engineer/items/ad5313d84da82c6ac421)
- [[Question] what advantage does axios give us over fetch? EOM](https://github.com/axios/axios/issues/314)
- [React NativeのHTTPリクエストにはfetchよりaxiosを使ったほうが安定しそう](http://mmiyauchi.com/?p=1883)

## テスト
- [vue-test-utils](https://vue-test-utils.vuejs.org/ja/)
- [JavaScriptテスト自動化キホンのキ](https://qiita.com/y-tsuna/items/6b8b824e444030070754)
- [フロントエンドにテストを導入](https://qiita.com/howdy39/items/cdd5b252096f5a2fa438)
- [Karma + PhantomJS + Mocha + power-assertでクライアントサイドJSのユニットテスト環境を構築してみた](https://liginc.co.jp/187209)
